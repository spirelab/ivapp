...
class SupportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_CREATE_TICKET', $msg, route('admin.ticket.view', $ticket->id));
        $this->userFirebasePushNotification($user, 'USER_NOTIFY_CREATE_TICKET', $msg, route('user.ticket.list'));
    ...
    public function reply(Request $request, $id)
    {
        ...
        if ($request->replayTicket == 1) {
            ...
            $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_REPLY_TICKET', $msg, route('admin.ticket.view', $ticket->id));
            ...
            $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_TICKET_CLOSE', $msg, route('admin.ticket.view', $ticket->id));
    ...
    public function saveTicket(Request $request, $random): Ticket
    {
        $purifiedData = Purify::clean($request->except('_token', '_method'));
        $ticket = new Ticket();
        $ticket->user_id = $this->user->id??auth()->id();
        $ticket->name = $this->user->username??auth()->user()->username;
        $ticket->email = $this->user->email??auth()->user()->email;
