...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['firebaseNotify'] = fireBaseControl();
    ...
    public function updateProfile(Request $request)
    {
        ...
        $this->userFirebasePushNotification($user, 'USER_NOTIFY_HIS_PROFILE_UPDATE', $msg, route('user.profile'));
    ...
    public function updateInformation(Request $request)
    {
        ...
        $this->userFirebasePushNotification($user, 'USER_NOTIFY_HIS_PROFILE_INFORMATION_UPDATE', $msg, route('user.profile'));
    ...
    public function updatePassword(Request $request)
    {
        ...
        try {
            if (Hash::check($request->current_password, $user->password)) {
                ...
                $this->userFirebasePushNotification($user, 'USER_NOTIFY_HIS_PROFILE_PASSWORD_UPDATE', $msg, route('user.profile'));
    ...
    public function purchasePlan(Request $request)
    {
        ...
        $this->adminFirebasePushNotification('PLAN_PURCHASE_NOTIFY_TO_ADMIN', $msg, route('admin.user.plan-purchaseLog', $user->id));
    ...
    public function userNotify($user, $withdraw)
    {
        try {
            ...
            $this->adminFirebasePushNotification($user, 'USER_NOTIFY_PAYOUT_REQUEST', $msg, route('admin.payout-request', $user->id));
            $this->userFirebasePushNotification('ADMIN_NOTIFY_PAYOUT_REQUEST', $msg, route('user.payout.history'));
        ...
        if ($user[$wallet_type] >= ($request->amount + $transferCharge)) {
            ...
            if (Hash::check($request->password, $user->password)) {
                ...
                if (isset($sendMoneyCheck) && Carbon::parse($sendMoneyCheck->send_at) > Carbon::now()) {
                    ...
                    $this->adminFirebasePushNotification('ADMIN_NOTIFY_BALANCE_TRANSFER', $msg);
                    $this->userFirebasePushNotification($user, 'SENDER_NOTIFY_BALANCE_TRANSFER', $msg, route('user.home'));
                    $this->userFirebasePushNotification($receiver, 'RECEIVER_NOTIFY_BALANCE_TRANSFER', $msg, route('user.home'));
    ...
    public function verificationSubmit(Request $request)
    {
        ...
        try {
            ...
            $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_KYC_REQUEST', $msg, route('admin.kyc.users.pending'));
            $this->userFirebasePushNotification($user, 'USER_NOTIFY_HIS_KYC_REQUEST_SEND', $msg, route('user.profile'));
    ...
    public function addressVerification(Request $request)
    {
        ...
        try {
            ...
            $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_ADDRESS_VERIFICATION_REQUEST', $msg, route('admin.kyc.users.pending'));
            $this->userFirebasePushNotification($user, 'USER_NOTIFY_ADDRESS_VERIFICATION_REQUEST_SEND', $msg, route('user.profile'));
