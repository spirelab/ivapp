...
function basicControl()
{
    ...
    if (!$general) {
        $general = Configure::firstOrCreate(['id' => 1]);
...
{
    $general = Cache::get('FireBaseNotifySet');
    if (!$general) {
        $general = \App\Models\FireBaseNotify::firstOrCreate(['id' => 1]);
        Cache::put('FireBaseNotifySet', $general);
    }
    return $general;
}
function smsConfig()
{
    $general = Cache::get('SmsNotifySet');
    if (!$general) {
        $general = \App\Models\SmsControl::firstOrCreate(['id' => 1]);
        Cache::put('SmsNotifySet', $general);
    }
    return $general;
}
...
{
    if (0 < $length) {
        return number_format($amount + 0, ($length != 0)??2);
    }
    return $amount + 0;
}
