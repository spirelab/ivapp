...
class BasicService
{
    ...
    public function preparePaymentUpgradation($order)
    {
        ...
        if ($order->status == 0) {
            ...
            if ($order->plan_id) {
                ...
                $this->userFirebasePushNotification($user, 'USER_NOTIFY_FUND_DEPOSIT_PAYMENT_COMPLETE', $msg, route('user.fund-history'));
    ...
    public function setBonus($user, $amount, $commissionType = ''){
        ...
        while ($userId != "" || $userId != "0" || $i < $level) {
            ...
            $this->userFirebasePushNotification($refer,'REFERRAL_BONUS', $msg, route('user.referral.bonus'));
